...
<div style='font-size:13px;opacity:0.92;'>Lineage heat: 0 outbound / 3 inbound · ✅ No engine drift detected · Index coverage: ready · PII: 1 · Changelog: fresh (2 d)</div>
</div>
...
- Pkg folder: [packages\peer-nodes](https://github.com/blackcatacademy/blackcat-database/blob/main/packages\peer-nodes)
...
graph LR
  ...
  peer_nodes["peer_nodes"]:::center
  replication_lag_samples["replication_lag_samples"]:::inbound -->|FK| peer_nodes
  sync_batches["sync_batches"]:::inbound -->|FK| peer_nodes
  sync_errors["sync_errors"]:::inbound -->|FK| peer_nodes
...
- Inbound (relies on this): "replication_lag_samples", "sync_batches", "sync_errors"
...
## Usage
- Install/upgrade schema: pwsh -NoLogo -NoProfile -File scripts/schema-tools/Migrate-DryRun.ps1 -Package peer-nodes -Apply
