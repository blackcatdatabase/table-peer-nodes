...
namespace BlackCat\Database\Packages\PeerNodes\Repository;

use BlackCat\Core\Database as Database;              // e.g. BlackCat\Core\Database
use BlackCat\Database\Packages\PeerNodes\Definitions;
use BlackCat\Database\Packages\PeerNodes\Criteria;
use BlackCat\Database\Packages\PeerNodes\Dto\PeerNodeDto as Dto;
use BlackCat\Database\Packages\PeerNodes\Mapper\PeerNodeDtoMapper as RowMapper;
...
final class PeerNodeRepository implements RepoContract, KeysetRepoContract
{
     ...
     */
    protected function def(): string { return \BlackCat\Database\Packages\PeerNodes\Definitions::class; }
    ...
    private function doUpsert(array $row, bool $revive): void
    {
        ...
        $updCols = [ 'type', 'location', 'status', 'last_seen', 'meta' ];
    ...
    public function upsertMany(array $rows): int {
        ...
        if (class_exists(\BlackCat\Database\BulkUpsertRepository::class) && [ 'name' ] !== []) {
            $bulk = new \BlackCat\Database\BulkUpsertRepository($this->db, \BlackCat\Database\Packages\PeerNodes\Definitions::table());
            return $bulk->upsertMany($rows, [ 'name' ], [ 'type', 'location', 'status', 'last_seen', 'meta' ]);
        }
    ...
    */
    ...
    /** @return array<string,mixed>|\BlackCat\Database\Packages\PeerNodes\Dto\PeerNodeDto|null */
    public function getByName(string $name, bool $asDto = false): array|\BlackCat\Database\Packages\PeerNodes\Dto\PeerNodeDto|null {
        $row = $this->getByUnique([ 'name' => $name ]);
        if (!$asDto || !$row) return $row;
        return \BlackCat\Database\Packages\PeerNodes\Mapper\PeerNodeDtoMapper::fromRow($row);
    }
    ...
    /** @return array<string,mixed>|\BlackCat\Database\Packages\PeerNodes\Dto\PeerNodeDto|null */
    public function getByConstraint(mixed $constraint, bool $asDto = false): array|\BlackCat\Database\Packages\PeerNodes\Dto\PeerNodeDto|null {
    ...
    /** @return array<string,mixed>|\BlackCat\Database\Packages\PeerNodes\Dto\PeerNodeDto|null */
    public function getById(int $id, bool $asDto = false): array|\BlackCat\Database\Packages\PeerNodes\Dto\PeerNodeDto|null {
