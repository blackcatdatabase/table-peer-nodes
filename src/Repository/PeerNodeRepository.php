...
use BlackCat\Database\Packages\PeerNodes\Repository\PeerNodeRepositoryInterface;
...
class PeerNodeRepository implements PeerNodeRepositoryInterface, RepoContract, KeysetRepoContract
{
use OrderByTools, PkTools, RepositoryHelpers;
    ...
    public function upsertMany(array $rows): int {
        ...
        if ($helperKeys && class_exists(\BlackCat\Database\Support\BulkUpsertHelper::class)) {
          $bulk = new \BlackCat\Database\Support\BulkUpsertHelper($this->db, \BlackCat\Database\Packages\PeerNodes\Definitions::class);
          $bulk->upsertMany($rows, $helperKeys, [ 'type', 'location', 'status', 'last_seen', 'meta' ]);
          return count($rows);
        }
