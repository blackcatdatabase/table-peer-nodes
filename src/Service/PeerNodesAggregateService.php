...
namespace BlackCat\Database\Packages\PeerNodes\Service;

use BlackCat\Core\Database;
use BlackCat\Database\Packages\PeerNodes\Dto\PeerNodeDto;
use BlackCat\Database\Packages\PeerNodes\Mapper\PeerNodeDtoMapper;
 ...
 */
final class PeerNodesAggregateService
{
    use ServiceHelpers;
    ...
    public function __construct(
        private Database $db, private PeerNodeRepository $peerNodeRepo
    ) {
        // Repository autowiring (generated)
        $this->repositories = [
  'peer_nodes' => $this->peerNodeRepo
];
    }
    ...
    protected function withRowLock(callable $locker, int|string|array $id, callable $fetch, string $mode = 'wait'): mixed
    {
        ...
        return $this->txn(function () use ($locker, $id, $fetch, $mode) {
            ...
            if (!$row) {
                ...
                throw new \BlackCat\Database\Packages\PeerNodes\ModuleException("Record {$idStr} not found for locking.");
            }
